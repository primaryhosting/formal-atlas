$( tests/fixtures/set_mm_head.mm - PROVENANCE

   First 150,001 bytes (curl -r 0-150000) of upstream set.mm, fetched
   2026-08-27 from:
     https://raw.githubusercontent.com/metamath/set.mm/develop/set.mm
   then truncated at the last complete TOP-LEVEL statement (outside any
   comment and outside any scoping block), which is `con2b`. Nothing else
   was altered: everything below this comment is verbatim upstream text.

   Golden expectations (verified by an independent token counter and by
   eye, see tests/test_metamath_golden.py): 362 statements = 354 $p
   (theorem) + 8 $a (wn wi wb ax-mp ax-1 ax-2 ax-3 -> axiom; df-bi ->
   definition). First statement: idi. Last statement: con2b. $)
...
$)
  ...
  $( Weakening of ~ ax-1 .  As a consequence, its associated inference is an
     ...
     11-Aug-2020.) $)
  ax1w $p |- ( ph -> ( ps -> ( ch -> ps ) ) ) $=
    ( wi ax-1 a1i ) BCBDDABCEF $.
    $( $j usage 'ax1w' avoids 'ax-2' 'ax-3'; $)
...
$)
  ...
  $( The Inversion Axiom of the infinite-valued sentential logic (L-infinity)
     ...
     [WhiteheadRussell] p. 108.  It is a special instance of the axiom "Roll",
     see ~ peirceroll .  (Contributed by NM, 12-Aug-2004.) $)
